...
abstract contract ReexecutionEndpointStorage is IReexecutionEndpoint {
    ...
    mapping(uint256 requestIndex => mapping(bytes32 response => uint256 stakeWeight)) internal _responseStakeWeights;

    // Mapping from reservationID => request history using DefaultZeroHistory
    mapping(uint256 reservationID => Snapshots.DefaultZeroHistory) internal _cumulativeReservationRequests;
