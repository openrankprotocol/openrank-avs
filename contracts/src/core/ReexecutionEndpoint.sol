...
import "../interfaces/core/IReexecutionEndpoint.sol";
import "../interfaces/core/IReservationRegistry.sol";
 ...
 */
contract ReexecutionEndpoint is PermissionControllerMixin, OwnableUpgradeable, ReexecutionEndpointStorage {
    ...
    function requestReexecution(uint32 imageID, bytes calldata requestData) external returns (uint256 requestIndex) {
        ...
        IReservationRegistry.Reservation memory reservation = reservationRegistry.getReservation(reservationID);
        ...
        uint256 currentRequests = _cumulativeReservationRequests[reservationID].latest();
        {
            ...
            uint256 requestsAtWindowStart = _cumulativeReservationRequests[reservationID].upperLookup(windowStartBlock);
        ...
        _cumulativeReservationRequests[reservationID].push(uint32(block.number), currentRequests + 1);
    ...
    function respond(
        ...
        require(block.number <= request.requestBlock + responseWindowBlocks, ResponseDeadlinePassed());
        ...
        require(!_operatorResponses[requestIndex].contains(operatorAddressAsBytes32), OperatorAlreadyResponded());

        uint256 operatorStakeWeight = uint256(_getOperatorWeight(request.epochStartBlockNumber, operator));
    ...
    function _verifyOperatorRegisteredDuringRequest(uint32 requestBlock, address operator) internal view {
        bytes32[] memory operatorIds = new bytes32[](1);
        operatorIds[0] = slashingRegistryCoordinator.getOperatorId(operator);

        uint32[] memory quorumBitmapIndices = slashingRegistryCoordinator.getQuorumBitmapIndicesAtBlockNumber({
    ...
    function _getTotalStakeWeight(
        ...
        uint32[] memory indices =
            stakeRegistry.getTotalStakeIndicesAtBlockNumber({ blockNumber: blocknumber, quorumNumbers: quorumNumbers });
    ...
    function _getOperatorWeight(
        ...
        operatorStakeWeight =
            stakeRegistry.getStakeAtBlockNumber(operatorId, RxpConstants.OPERATOR_SET_ID_UINT8, blocknumber);
    }
    ...
    /// @inheritdoc IReexecutionEndpoint
    function hasOperatorResponded(uint256 requestIndex, address operator) public view returns (bool) {
        require(requestIndex < _requests.length, InvalidRequestIndex());
        bytes32 operatorAddressAsBytes32 = bytes32(uint256(uint160(operator)));
    ...
    function getResponse(uint256 requestIndex, address operator) external view returns (bytes32 response) {
        require(requestIndex < _requests.length, InvalidRequestIndex());
        bytes32 operatorAddressAsBytes32 = bytes32(uint256(uint160(operator)));
        require(_operatorResponses[requestIndex].contains(operatorAddressAsBytes32), OperatorHasNotResponded());
