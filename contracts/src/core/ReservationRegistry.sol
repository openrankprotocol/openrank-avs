 ...
 */
contract ReservationRegistry is PermissionControllerMixin, OwnableUpgradeable, ReservationRegistryStorage {
    ...
    /// @inheritdoc IReservationRegistry
    function reserve(address avs, uint256 transferAmount) external checkCanCall(avs) returns (uint256 reservationID) {
        ...
        _reservations[reservationID] =
            Reservation({ avs: avs, balance: transferAmount, lastDeductionEpoch: currentEpoch(), active: true });
    ...
    function addImage(uint256 reservationID, bytes[] calldata imageDACerts) external returns (uint32 imageID) {
        ...
        require(
            _reservationImageIDs[reservationID].length() < maxImagesPerReservation, MaxImagesPerReservationReached()
        );
        ...
        _images[imageID] =
            Image({ reservationID: reservationID, imageDACerts: imageDACerts, creationTime: uint32(block.timestamp) }); // Updated Image struct
    ...
    function deductFees(
        ...
        for (uint256 i = 0; i < reservationIDs.length; i++) {
            ...
            if (numPassedEpochs > 0) {
                uint256 billAmount = resourceCostPerOperatorPerEpoch * currentNumOperators * numPassedEpochs;
