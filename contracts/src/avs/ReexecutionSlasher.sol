...
import { ICertificateVerifier } from "../interfaces/avs/ICertificateVerifier.sol";
import { IReexecutionSlasher } from "../interfaces/avs/IReexecutionSlasher.sol";
import { IERC20, IReexecutionEndpoint } from "../interfaces/core/IReexecutionEndpoint.sol";
import { IReservationRegistry } from "../interfaces/core/IReservationRegistry.sol";
import {
    IIndexRegistry,
...
contract ReexecutionSlasher is SlasherBase, Initializable, IReexecutionSlasher {
    ...
    function requestReexecution(
        ...
        uint256 requestIndex = reexecutionEndpoint.requestReexecution(taskResponse.imageID, taskResponse.inputData);
    ...
    function processFinalizedReexecutionRequest(
...

        {
            ...
            require(
                operatorCount == signerOperatorIds.length + nonSignerOperatorIds.length, InvalidOperatorInputParams()
            );

            // verify that the nonsigners in calldata match the nonsigners in the certificate
            bytes32 signatoryRecordHash =
                keccak256(abi.encodePacked(verificationRecord.referenceBlockNumber, nonSignerOperatorIds));
            require(signatoryRecordHash == verificationRecord.signatoryRecordHash, InvalidNonsignerOperatorIds());
        }
