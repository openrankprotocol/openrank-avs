...
import "../../src/core/ReexecutionEndpoint.sol";
...
import "./DeployTestUtils.sol";
import { IStrategy } from "eigenlayer-middleware/src/StakeRegistry.sol";
import { MinimalServiceManager } from "teal-contracts/src/MinimalServiceManager.sol";
/**
 * @title DeployRxp script for deploying the ReexecutionEndpoint and ReservationRegistry and its AVS Middleware contracts
 *
 *
 *   forge script script/deploy/deploy_from_scratch.s.sol:DeployFromScratch --rpc-url $RPC_URL \
 *     --private-key $PRIVATE_KEY -vvvv \
 *     --sig "run(string memory configFile)" -- script/config/deploy_from_scratch.config.json
 */
...
contract BaseDeployRxp is DeployTestUtils {
    ...
    function run(bool broadcast, string memory environment) public {
        string memory rxpConfigPath = string.concat("script/", environment, "/config/deploy_rxp_contracts.config.json");
        string memory eigenlayerContractsPath =
            string.concat("script/", environment, "/output/deploy_eigenlayer_core_output.json");
        string memory outputPath = string.concat("script/", environment, "/output/deploy_rxp_contracts_output.json");
    ...
    function _deployContracts() internal {
        ...
        IEigenDACertVerifier certificateVerifier = IEigenDACertVerifier(address(0));
        ...
        socketRegistry =
            ISocketRegistry(address(new TransparentUpgradeableProxy(address(emptyContract), address(proxyAdmin), "")));
        ...
        paymentToken = new ERC20PresetFixedSupply("RXP MOCK", "RXP MOCK", 1000000000 ether, initialOwner);
        ...
        indexRegistryImplementation = new IndexRegistry(slashingRegistryCoordinator);
        ...
        apkRegistryImplementation = new BLSApkRegistry(slashingRegistryCoordinator);
        ...
        proxyAdmin.upgrade(
            ITransparentUpgradeableProxy(payable(address(apkRegistry))), address(apkRegistryImplementation)
        );

        proxyAdmin.upgrade(
            ITransparentUpgradeableProxy(payable(address(socketRegistry))), address(socketRegistryImplementation)
        );

        proxyAdmin.upgradeAndCall(
            ITransparentUpgradeableProxy(payable(address(serviceManager))),
            address(serviceManagerImplementation),
            abi.encodeWithSelector(MinimalServiceManager.initialize.selector, initialOwner, initialOwner)
        );
        ...
        allocationManager.updateAVSMetadataURI(address(serviceManager), "reexecution test");
        ...
        ISlashingRegistryCoordinatorTypes.OperatorSetParam memory operatorSetParams = ISlashingRegistryCoordinatorTypes
            .OperatorSetParam({ maxOperatorCount: uint32(200), kickBIPsOfOperatorStake: 11000, kickBIPsOfTotalStake: 1001 });
        IStakeRegistryTypes.StrategyParams[] memory strategyParams = new IStakeRegistryTypes.StrategyParams[](1);
        strategyParams[0] = IStakeRegistryTypes.StrategyParams({ strategy: rxpStrategy, multiplier: 1e18 });

        slashingRegistryCoordinator.createTotalDelegatedStakeQuorum(
            operatorSetParams, 1e18, /* minimumStake */ strategyParams
        );
        ...
        allocationManager.setAVSRegistrar(address(serviceManager), IAVSRegistrar(address(slashingRegistryCoordinator)));
    }

    function _parseRxpConfig(
        ...
        //proxyAdmin
        proxyAdmin = ProxyAdmin(stdJson.readAddress(config_data, ".proxyAdmin"));
        emptyContract = EmptyContract(stdJson.readAddress(config_data, ".emptyContract"));
        initialOwner = stdJson.readAddress(config_data, ".initialOwner");
        // ReexecutionEndpoint
        responseFeePerOperator = stdJson.readUint(config_data, ".ReexecutionEndpoint.responseFeePerOperator");
        reexecutionFeePerOperator = stdJson.readUint(config_data, ".ReexecutionEndpoint.reexecutionFeePerOperator");
        responseWindowBlocks = stdJson.readUint(config_data, ".ReexecutionEndpoint.responseWindowBlocks");
        maximumRequestsPerReservationPerResponseWindow =
            stdJson.readUint(config_data, ".ReexecutionEndpoint.maximumRequestsPerReservationPerResponseWindow");

        // ReservationRegistry
        operatorFeeDistributor = stdJson.readAddress(config_data, ".ReservationRegistry.operatorFeeDistributor");
        epochLengthBlocks = stdJson.readUint(config_data, ".ReservationRegistry.epochLengthBlocks");
        epochGenesisBlock = stdJson.readUint(config_data, ".ReservationRegistry.epochGenesisBlock");
        prepaidBilledEpochs = stdJson.readUint(config_data, ".ReservationRegistry.prepaidBilledEpochs");
        resourceCostPerOperatorPerEpoch =
            stdJson.readUint(config_data, ".ReservationRegistry.resourceCostPerOperatorPerEpoch");
        maxImagesPerReservation = stdJson.readUint(config_data, ".ReservationRegistry.maxImagesPerReservation");
        maxReservations = stdJson.readUint(config_data, ".ReservationRegistry.maxReservations");
        reservationBondAmount = stdJson.readUint(config_data, ".ReservationRegistry.reservationBondAmount");
    }

    /// @notice parse the existing Rxp contracts. Used for upgrade
    function _parseRxpContracts(
        string memory rxpContractsPath
    ) internal {
        // READ JSON CONFIG DATA FOR EIGENLAYER CONTRACTS
        string memory rxp_config_data = vm.readFile(rxpContractsPath);

        // Read addresses
        proxyAdmin = ProxyAdmin(stdJson.readAddress(rxp_config_data, ".addresses.proxyAdmin"));
        emptyContract = EmptyContract(stdJson.readAddress(rxp_config_data, ".addresses.emptyContract"));

        // Read ReexecutionEndpoint addresses
        reexecutionEndpoint =
            IReexecutionEndpoint(stdJson.readAddress(rxp_config_data, ".addresses.reexecutionEndpoint.proxy"));
        reexecutionEndpointImplementation =
            IReexecutionEndpoint(stdJson.readAddress(rxp_config_data, ".addresses.reexecutionEndpoint.implementation"));

        // Read ReservationRegistry addresses
        reservationRegistry =
            IReservationRegistry(stdJson.readAddress(rxp_config_data, ".addresses.reservationRegistry.proxy"));
        reservationRegistryImplementation =
            IReservationRegistry(stdJson.readAddress(rxp_config_data, ".addresses.reservationRegistry.implementation"));
        paymentToken = IERC20(stdJson.readAddress(rxp_config_data, ".addresses.reservationRegistry.paymentToken"));

        // Read AVS Middleware addresses
        indexRegistry = IIndexRegistry(stdJson.readAddress(rxp_config_data, ".addresses.indexRegistry"));
        stakeRegistry = IStakeRegistry(stdJson.readAddress(rxp_config_data, ".addresses.stakeRegistry"));
        apkRegistry = IBLSApkRegistry(stdJson.readAddress(rxp_config_data, ".addresses.apkRegistry"));
        socketRegistry = ISocketRegistry(stdJson.readAddress(rxp_config_data, ".addresses.socketRegistry"));
        serviceManager = IServiceManager(stdJson.readAddress(rxp_config_data, ".addresses.serviceManager"));
        slashingRegistryCoordinator =
            ISlashingRegistryCoordinator(stdJson.readAddress(rxp_config_data, ".addresses.slashingRegistryCoordinator"));
        avsPauserReg = PauserRegistry(stdJson.readAddress(rxp_config_data, ".addresses.avsPauserReg"));
    }
    ...
    function _parseEigenLayerContracts(
        ...
        // READ JSON CONFIG DATA FOR EIGENLAYER CONTRACTS
        string memory eigenlayer_config_data = vm.readFile(eigenlayerContractsPath);

        // PermissionController - read just the address, not the implementation
        permissionController =
            IPermissionController(stdJson.readAddress(eigenlayer_config_data, ".addresses.permissionController"));
        delegationManager =
            IDelegationManager(stdJson.readAddress(eigenlayer_config_data, ".addresses.delegationManager"));
        avsDirectory = IAVSDirectory(stdJson.readAddress(eigenlayer_config_data, ".addresses.avsDirectory"));
        allocationManager =
            IAllocationManager(stdJson.readAddress(eigenlayer_config_data, ".addresses.allocationManager"));
        rewardsCoordinator =
            IRewardsCoordinator(stdJson.readAddress(eigenlayer_config_data, ".addresses.rewardsCoordinator"));
        strategyManager = IStrategyManager(stdJson.readAddress(eigenlayer_config_data, ".addresses.strategyManager"));
        strategyFactory = IStrategyFactory(stdJson.readAddress(eigenlayer_config_data, ".addresses.strategyFactory"));
        eigenlayerProxyAdmin =
            ProxyAdmin(stdJson.readAddress(eigenlayer_config_data, ".addresses.eigenLayerProxyAdmin"));
        eigenlayerPauserReg =
            PauserRegistry(stdJson.readAddress(eigenlayer_config_data, ".addresses.eigenLayerPauserReg"));
    }
    ...
    function _writeOutputJSON(
...

        {
            ...
            vm.serializeAddress(addresses, "emptyContract", address(emptyContract));
            vm.serializeAddress(addresses, "permissionController", address(permissionController));

            // AVS Middleware addresses
            vm.serializeAddress(addresses, "indexRegistry", address(indexRegistry));
            vm.serializeAddress(addresses, "stakeRegistry", address(stakeRegistry));
            vm.serializeAddress(addresses, "apkRegistry", address(apkRegistry));
            vm.serializeAddress(addresses, "socketRegistry", address(socketRegistry));
            vm.serializeAddress(addresses, "serviceManager", address(serviceManager));
            vm.serializeAddress(addresses, "slashingRegistryCoordinator", address(slashingRegistryCoordinator));
            vm.serializeAddress(addresses, "avsPauserReg", address(avsPauserReg));

            // Create operatorSet object
            string memory operator_set = "operatorSet";
            vm.serializeAddress(operator_set, "rxpStrategy", address(rxpStrategy));
            string memory operator_set_output =
                vm.serializeAddress(operator_set, "underlyingToken", address(paymentToken));
            ...
            vm.serializeAddress(reexecution_endpoint, "proxy", address(reexecutionEndpoint));
            string memory reexecution_output =
                vm.serializeAddress(reexecution_endpoint, "implementation", address(reexecutionEndpointImplementation));

            // ReservationRegistry addresses
            string memory reservation_registry = "reservation_registry";
            vm.serializeAddress(reservation_registry, "proxy", address(reservationRegistry));
            vm.serializeAddress(reservation_registry, "implementation", address(reservationRegistryImplementation));
            vm.serializeAddress(reservation_registry, "paymentToken", address(paymentToken));
            vm.serializeAddress(reservation_registry, "rxpStrategy", address(rxpStrategy));
            string memory reservation_output =
                vm.serializeAddress(reservation_registry, "operatorFeeDistributor", operatorFeeDistributor);

            // Add nested objects to addresses
            vm.serializeString(addresses, "reexecutionEndpoint", reexecution_output);
            string memory addresses_output = vm.serializeString(addresses, "reservationRegistry", reservation_output);

            // Write parameters
            string memory parameters = "parameters";
            vm.serializeAddress(parameters, "initialOwner", initialOwner);
            vm.serializeUint(parameters, "responseFeePerOperator", responseFeePerOperator);
            vm.serializeUint(parameters, "reexecutionFeePerOperator", reexecutionFeePerOperator);
            vm.serializeUint(parameters, "responseWindowBlocks", responseWindowBlocks);
            vm.serializeUint(
                parameters,
                "maximumRequestsPerReservationPerResponseWindow",
                maximumRequestsPerReservationPerResponseWindow
            );
            vm.serializeUint(parameters, "prepaidBilledEpochs", prepaidBilledEpochs);
            vm.serializeUint(parameters, "resourceCostPerOperatorPerEpoch", resourceCostPerOperatorPerEpoch);
            vm.serializeUint(parameters, "maxImagesPerReservation", maxImagesPerReservation);
            vm.serializeUint(parameters, "epochLengthBlocks", epochLengthBlocks);
            vm.serializeUint(parameters, "epochGenesisBlock", epochGenesisBlock);
            vm.serializeUint(parameters, "reservationBondAmount", reservationBondAmount);
            string memory parameters_output = vm.serializeUint(parameters, "maxReservations", maxReservations);
            ...
            finalJson = vm.serializeString(parent_object, "chainInfo", chain_info_output);
        }
    ...
    function upgradeExistingContracts(bool broadcast, string memory environment) public {
        string memory rxpConfigPath = string.concat("script/", environment, "/config/deploy_rxp_contracts.config.json");
        // both the contracts path and output path are the same
        string memory rxpContractsPath =
            string.concat("script/", environment, "/output/deploy_rxp_contracts_output.json");
        _parseRxpConfig(rxpConfigPath);
