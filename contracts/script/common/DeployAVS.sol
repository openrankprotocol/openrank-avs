...
abstract contract DeployAVS is DeployTestUtils {
    ...
    function run(
        ...
        eigenlayerDeployment = parseConfig(
            inputConfigPath
        );
    ...
    function parseConfig(
        ...
        public
        virtual
        returns (EigenlayerDeployment memory deployment)
    {
        // read the json file
        string memory inputConfig = vm.readFile(inputConfigPath);
        deployment = EigenlayerDeployment({
        ...
        emit log_named_address(
            ...
            deployment.permissionController
        );
        emit log_named_address(
            "rewards coordinator",
            deployment.rewardsCoordinator
        );
        emit log_named_address(
            "avs directory",
            deployment.avsDirectory
        );
    }
}
