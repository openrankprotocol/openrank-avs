...
import { IERC20, IReexecutionEndpoint } from "../../src/interfaces/core/IReexecutionEndpoint.sol";
...
import { CertificateVerifier } from "../../src/avs/CertificateVerifier.sol";
import { ReexecutionSlasher } from "../../src/avs/ReexecutionSlasher.sol";
...
abstract contract BaseDeploySimpleAVS is DeployAVS, DeployTestUtils {
    ...
    function run(bool broadcast, string memory _environment) public virtual {
        ...
        eigenlayerConfigPath = string.concat("script/", environment, "/output/deploy_eigenlayer_core_output.json");
        rxConfigPath = string.concat("script/", environment, "/output/deploy_rxp_contracts_output.json");
    ...
    function _deploySimpleAVS() internal {
        ...
        rxCertificateVerifierImplementation = new CertificateVerifier(slashingRegistryCoordinator, reservationRegistry);
        ...
        avsProxyAdmin.upgradeAndCall(
            ...
            abi.encodeWithSelector(ReexecutionSlasher.initialize.selector, 0 /* operatorSetId */ )
        );

        avsProxyAdmin.upgrade(
            ITransparentUpgradeableProxy(address(rxCertificateVerifier)), address(rxCertificateVerifierImplementation)
        );
        ...
        serviceManager.setAppointee(
            address(reexecutionSlasher), address(reexecutionEndpoint), reexecutionEndpoint.requestReexecution.selector
        );

        // 2. set Rx Slasher as appointee for calling AllocationManager.slashOperator
        serviceManager.setAppointee(
            address(reexecutionSlasher), address(allocationManager), allocationManager.slashOperator.selector
        );

        // 3. set owner as appointee for calling RxP's ReservationRegistry interfaces
        serviceManager.setAppointee(msg.sender, address(reservationRegistry), reservationRegistry.reserve.selector);

        // 4. set owner as appointee for calling RxP's ReservationRegistry.addImage
        serviceManager.setAppointee(msg.sender, address(reservationRegistry), reservationRegistry.addImage.selector);

        // 5. set owner as appointee for calling RxP's ReservationRegistry.removeImage
        serviceManager.setAppointee(msg.sender, address(reservationRegistry), reservationRegistry.removeImage.selector);

        // 6. set owner as appointee for calling RxP's ReservationRegistry.addFunds
        serviceManager.setAppointee(msg.sender, address(reservationRegistry), reservationRegistry.addFunds.selector);

        // 7. set owner as appointee for calling RxP's ReservationRegistry.deductFees
        serviceManager.setAppointee(msg.sender, address(reservationRegistry), reservationRegistry.deductFees.selector);
    }

    function parseConfig(
        string memory eigenlayerConfigPath
    ) public virtual override returns (EigenlayerDeployment memory eigenlayerDeployment) {
        // read the json file
        string memory inputConfig = vm.readFile(eigenlayerConfigPath);

        eigenlayerDeployment = EigenlayerDeployment({
            allocationManager: stdJson.readAddress(inputConfig, ".addresses.allocationManager"),
            delegationManager: stdJson.readAddress(inputConfig, ".addresses.delegationManager"),
            permissionController: stdJson.readAddress(inputConfig, ".addresses.permissionController"),
            rewardsCoordinator: stdJson.readAddress(inputConfig, ".addresses.rewardsCoordinator"),
            avsDirectory: stdJson.readAddress(inputConfig, ".addresses.avsDirectory")
        });

        allocationManager = IAllocationManager(eigenlayerDeployment.allocationManager);
        delegationManager = IDelegationManager(eigenlayerDeployment.delegationManager);
        permissionController = IPermissionController(eigenlayerDeployment.permissionController);
        rewardsCoordinator = IRewardsCoordinator(eigenlayerDeployment.rewardsCoordinator);
    ...
    function parseRxConfig() public virtual {
        ...
        reservationRegistry =
            IReservationRegistry(stdJson.readAddress(inputConfig, ".addresses.reservationRegistry.proxy"));
        reexecutionEndpoint =
            IReexecutionEndpoint(stdJson.readAddress(inputConfig, ".addresses.reexecutionEndpoint.proxy"));
        rxpStrategy = IStrategy(stdJson.readAddress(inputConfig, ".addresses.reservationRegistry.rxpStrategy"));
    }
     ...
     */
    function chooseStrategiesForAVS() public view virtual returns (IStrategy[] memory);
    ...
    function _writeOutputJSON() internal virtual {
        ...
        vm.serializeAddress(output, "certificateVerifier", address(rxCertificateVerifier));
        ...
        address[] memory strategyAddresses = new address[](avsStrategies.length);
        ...
        vm.serializeAddress(output, "reexecutionSlasher", address(reexecutionSlasher));
        vm.serializeAddress(output, "certificateVerifier", address(rxCertificateVerifier));
        string memory finalJson = vm.serializeString(output, "object", output);

        string memory outputDir = string.concat("script/", environment, "/output");
        string memory outputPath = string.concat(outputDir, "/deploy_simple_avs_output.json");
