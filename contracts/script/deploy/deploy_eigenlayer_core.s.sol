...
contract DeployEigenLayerCore is DeployTestUtils {
    ...
    function _deployEigenLayerContracts() internal {
        ...
        eigenPodImplementation = new EigenPod(ethPOSDeposit, eigenPodManager, GOERLI_GENESIS_TIME, "1.0.0");
        ...
        strategyManagerImplementation = new StrategyManager(delegation, eigenLayerPauserReg, "1.0.0");
        strategyFactoryImplementation = new StrategyFactory(strategyManager, eigenLayerPauserReg, "1.0.0");
        baseStrategyImplementation = new StrategyBase(strategyManager, eigenLayerPauserReg, "v1.0.0");
        avsDirectoryImplementation = new AVSDirectory(delegation, eigenLayerPauserReg, "1.0.0");
        eigenPodManagerImplementation =
            new EigenPodManager(ethPOSDeposit, eigenPodBeacon, delegation, eigenLayerPauserReg, "1.0.0");
        ...
        rewardsCoordinatorImplementation = new RewardsCoordinator(rewardsCoordinatorConstructorParams);
    ...
    function _parseConfig(
        ...
        deployConfigPath = string(bytes(string.concat("script/local/config/", configFileName)));
        ...
        STRATEGY_FACTORY_INIT_PAUSED_STATUS = stdJson.readUint(config_data, ".strategyFactory.init_paused_status");
        ...
        StrategyConfig[] memory stratConfigs = abi.decode(strategyConfigsRaw, (StrategyConfig[]));
    ...
    function _parseDeployedOutput() internal {
        string memory outputPath = "script/local/output/deploy_eigenlayer_core_output.json";
        ...
        delegation = DelegationManager(stdJson.readAddress(json, ".addresses.delegationManager"));
        delegationImplementation =
            DelegationManager(stdJson.readAddress(json, ".addresses.delegationManagerImplementation"));
        strategyManager = StrategyManager(stdJson.readAddress(json, ".addresses.strategyManager"));
        strategyManagerImplementation =
            StrategyManager(stdJson.readAddress(json, ".addresses.strategyManagerImplementation"));
        eigenPodManager = EigenPodManager(stdJson.readAddress(json, ".addresses.eigenPodManager"));
        eigenPodManagerImplementation =
            EigenPodManager(stdJson.readAddress(json, ".addresses.eigenPodManagerImplementation"));
        rewardsCoordinator = RewardsCoordinator(stdJson.readAddress(json, ".addresses.rewardsCoordinator"));
        rewardsCoordinatorImplementation =
            RewardsCoordinator(stdJson.readAddress(json, ".addresses.rewardsCoordinatorImplementation"));
        avsDirectory = AVSDirectory(stdJson.readAddress(json, ".addresses.avsDirectory"));
        avsDirectoryImplementation = AVSDirectory(stdJson.readAddress(json, ".addresses.avsDirectoryImplementation"));
        allocationManager = AllocationManager(stdJson.readAddress(json, ".addresses.allocationManager"));
        allocationManagerImplementation =
            AllocationManager(stdJson.readAddress(json, ".addresses.allocationManagerImplementation"));
        permissionController = PermissionController(stdJson.readAddress(json, ".addresses.permissionController"));
        permissionControllerImplementation =
            PermissionController(stdJson.readAddress(json, ".addresses.permissionControllerImplementation"));
        eigenPodBeacon = UpgradeableBeacon(stdJson.readAddress(json, ".addresses.eigenPodBeacon"));
        eigenPodImplementation = EigenPod(payable(stdJson.readAddress(json, ".addresses.eigenPodImplementation")));
        baseStrategyImplementation =
            StrategyBase(payable(stdJson.readAddress(json, ".addresses.baseStrategyImplementation")));
        emptyContract = EmptyContract(payable(stdJson.readAddress(json, ".addresses.emptyContract")));

        // Read parameters
        executorMultisig = stdJson.readAddress(json, ".parameters.executorMultisig");
        operationsMultisig = stdJson.readAddress(json, ".parameters.operationsMultisig");
        pauserMultisig = stdJson.readAddress(json, ".parameters.pauserMultisig");
    }
    ...
    function _writeOutputJSON() internal {
        ...
        vm.serializeAddress(deployed_addresses, "strategyBeaconImplementation", address(baseStrategyImplementation));
        ...
        string memory addresses_output =
        ...
        vm.writeJson(finalJson, "script/local/output/deploy_eigenlayer_core_output.json");
    }
}
