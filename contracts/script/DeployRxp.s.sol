...
import {ReexecutionEndpoint} from "../src/rxp/ReexecutionEndpoint.sol";
import {ReservationRegistry} from "../src/rxp/ReservationRegistry.sol";
import {IReservationRegistry} from "../src/rxp/interfaces/IReservationRegistry.sol";
import {IReexecutionEndpoint} from "../src/rxp/interfaces/IReexecutionEndpoint.sol";
...
import {RxpServiceManager} from "../src/rxp/RxpServiceManager.sol";
...
contract DeployRxp is DeployTestUtils {
    ...
    function run() public {
        ...
        string
            memory eigenLayerCoreOutputPath = "contracts/script/output/deploy_eigenlayer_core.json";
        string
            memory outputPath = "contracts/script/output/deploy_rxp_contracts.json";

        _parseEigenLayerContracts(eigenLayerCoreOutputPath);
    ...
    function _deployContracts() internal {
        ...
        serviceManagerImplementation = new RxpServiceManager(
            IAVSDirectory(avsDirectory),
            IRewardsCoordinator(rewardsCoordinator),
            slashingRegistryCoordinator,
            stakeRegistry,
        ...
        proxyAdmin.upgradeAndCall(
            ...
            abi.encodeWithSelector(
                RxpServiceManager.initialize.selector,
                initialOwner,
                initialOwner
            )
        );
        ...
        console.log("DeployRxpBase sender: ", msg.sender);
        console.log("DeployRxpBase: ", address(this));
